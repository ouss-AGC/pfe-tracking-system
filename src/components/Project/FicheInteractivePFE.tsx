...
import { Shield, MapPin, Calendar, User, FileText, Download } from 'lucide-react';
...
const FicheInteractivePFE: React.FC<FicheInteractivePFEProps> = ({ project }) => {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            pdf.save(`Fiche_Proposition_${project.nomEtudiant.replace(/\s+/g, '_')}.pdf`);
        } catch (error) {
    ...
    return (
        <div className="fiche-pfe-wrapper">
            <div className="fiche-export-header no-print">
                <button
                    className="btn btn-primary btn-sm"
                    onClick={handleExport}
                    disabled={isExporting}
                >
                    <Download size={16} /> {isExporting ? 'Génération...' : 'Télécharger en PDF'}
                </button>
            </div>
            <div className="fiche-pfe-container" ref={ficheRef}>
